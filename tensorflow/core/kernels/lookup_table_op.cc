...
//
template <class K, class V> class MutableHashTableOfScalars final : public LookupInterface {
public:
    ...
    Status Find(OpKernelContext *ctx, const Tensor &key, Tensor *value, const Tensor &default_value) override
    {
        ...
        for (int64 i = 0; i < key_values.size(); ++i) {
            value_values(i) = gtl::FindWithDefault(table_, SubtleMustCopyIfIntegral(key_values(i)), default_val);
        }
    ...
    Status ExportValues(OpKernelContext *ctx) override
    {
        ...
        Tensor *keys;
        Tensor *values;
        TF_RETURN_IF_ERROR(ctx->allocate_output("keys", TensorShape({ size }), &keys));
...
template <class K, class V> class MutableHashTableOfTensors final : public LookupInterface {
public:
    MutableHashTableOfTensors(OpKernelContext *ctx, OpKernel *kernel)
    {
        ...
        OP_REQUIRES(ctx, TensorShapeUtils::IsVector(value_shape_),
            errors::InvalidArgument("Default value must be a vector, got shape ", value_shape_.DebugString()));
    }
    ...
    Status Find(OpKernelContext *ctx, const Tensor &key, Tensor *value, const Tensor &default_value) override
    {
        ...
        tf_shared_lock l(mu_);
        for (int64 i = 0; i < key_values.size(); ++i) {
            ValueArray *value_vec = gtl::FindOrNull(table_, SubtleMustCopyIfIntegral(key_values(i)));
    ...
    Status DoInsert(bool clear, const Tensor &keys, const Tensor &values)
    {
        ...
        for (int64 i = 0; i < key_values.size(); ++i) {
            ...
            gtl::InsertOrUpdate(&table_, SubtleMustCopyIfIntegral(key_values(i)), value_vec);
        }
        return Status::OK();
    }
    ...
    Status ExportValues(OpKernelContext *ctx) override
    {
        ...
        Tensor *values;
        TF_RETURN_IF_ERROR(ctx->allocate_output("keys", TensorShape({ size }), &keys));
        TF_RETURN_IF_ERROR(ctx->allocate_output("values", TensorShape({ size, value_dim }), &values));
...
namespace {
template <typename T> inline uint64 HashScalar(const T &key)
{
...
template <class K, class V> class MutableDenseHashTable final : public LookupInterface {
public:
    MutableDenseHashTable(OpKernelContext *ctx, OpKernel *kernel)
    {
        OP_REQUIRES_OK(ctx, GetNodeAttr(kernel->def(), "max_load_factor", &max_load_factor_));
        OP_REQUIRES(ctx, max_load_factor_ > 0 && max_load_factor_ < 1,
            errors::InvalidArgument("max_load_factor must be between 0 and 1, got: ", max_load_factor_));

        OP_REQUIRES_OK(ctx, GetNodeAttr(kernel->def(), "value_shape", &value_shape_));
        OP_REQUIRES(ctx, TensorShapeUtils::IsScalar(value_shape_) || TensorShapeUtils::IsVector(value_shape_),
            errors::InvalidArgument("Empty value must be a scalar or a vector, got shape ",
            value_shape_.DebugString()));

        const Tensor *empty_key_input;
        OP_REQUIRES_OK(ctx, ctx->input("empty_key", &empty_key_input));
        key_shape_ = empty_key_input->shape();
        OP_REQUIRES(ctx, TensorShapeUtils::IsScalar(key_shape_) || TensorShapeUtils::IsVector(key_shape_),
            errors::InvalidArgument("Empty key must be a scalar or a vector, got shape ", key_shape_.DebugString()));
        empty_key_ = PersistentTensor(*empty_key_input);
        empty_key_hash_ = HashKey(empty_key_input->template shaped<K, 2>({ 1, key_shape_.num_elements() }), 0);
        ...
        OP_REQUIRES(ctx, key_shape_.IsSameSize(deleted_key_input->shape()),
            errors::InvalidArgument("Empty and deleted keys must have same shape, got shapes: ",
            key_shape_.DebugString(), " and ", deleted_key_input->shape().DebugString()));
        deleted_key_ = PersistentTensor(*deleted_key_input);
        deleted_key_hash_ = HashKey(deleted_key_input->template shaped<K, 2>({ 1, key_shape_.num_elements() }), 0);

        if (empty_key_hash_ == deleted_key_hash_) {
            const int64 key_size = key_shape_.num_elements();
            const auto empty_key_matrix = empty_key_.AccessTensor(ctx)->template shaped<K, 2>({ 1, key_size });
            const auto deleted_key_matrix = deleted_key_.AccessTensor(ctx)->template shaped<K, 2>({ 1, key_size });
            OP_REQUIRES(ctx, !IsEqualKey(empty_key_matrix, 0, deleted_key_matrix, 0),
                errors::InvalidArgument("Empty and deleted keys cannot be equal"));
        }

        int64 initial_num_buckets;
        OP_REQUIRES_OK(ctx, GetNodeAttr(kernel->def(), "initial_num_buckets", &initial_num_buckets));
        ...
        LOCKS_EXCLUDED(mu_)
    {
        ...
        if (key.NumElements() != num_elements * key_size) {
            ...
            return errors::InvalidArgument("Expected key shape ", expected_shape.DebugString(), " got ",
        ...
        tf_shared_lock l(mu_);
        const auto key_buckets_matrix = key_buckets_.AccessTensor(ctx)->template matrix<K>();
        const auto value_buckets_matrix = value_buckets_.AccessTensor(ctx)->template matrix<V>();
        const auto empty_key_matrix = empty_key_.AccessTensor(ctx)->template shaped<K, 2>({ 1, key_size });
        const auto deleted_key_matrix = deleted_key_.AccessTensor(ctx)->template shaped<K, 2>({ 1, key_size });
        ...
        for (int64 i = 0; i < num_elements; ++i) {
            ...
            if (empty_key_hash_ == key_hash && IsEqualKey(empty_key_matrix, 0, key_matrix, i)) {
                return errors::InvalidArgument("Using the empty_key as a table key is not allowed");
            }
            if (deleted_key_hash_ == key_hash && IsEqualKey(deleted_key_matrix, 0, key_matrix, i)) {
                return errors::InvalidArgument("Using the deleted_key as a table key is not allowed");
            }
            ...
            while (true) {
                if (IsEqualKey(key_buckets_matrix, bucket_index, key_matrix, i)) {
                    for (int64 j = 0; j < value_size; ++j) {
                        ...
                        value_matrix(i, j) = SubtleMustCopyIfIntegral(value_buckets_matrix(bucket_index, j));
                    }
                    break;
                }
                ...
                ++num_probes;
                bucket_index = (bucket_index + num_probes) & bit_mask; // quadratic probing
                if (num_probes >= num_buckets_) {
                    return errors::Internal("Internal error in MutableDenseHashTable lookup");
                }
            }
        }
        return Status::OK();
    ...
    Status Insert(OpKernelContext *ctx, const Tensor &key, const Tensor &value) override LOCKS_EXCLUDED(mu_)
    {
    ...
    Status Remove(OpKernelContext *ctx, const Tensor &key) override LOCKS_EXCLUDED(mu_)
    {
    ...
    Status ImportValues(OpKernelContext *ctx, const Tensor &keys, const Tensor &values) override LOCKS_EXCLUDED(mu_)
    {
        mutex_lock l(mu_);
        ...
        const auto empty_key_tensor =
            empty_key_.AccessTensor(ctx)->template shaped<K, 2>({ 1, key_shape_.num_elements() });
        const auto deleted_key_tensor =
            deleted_key_.AccessTensor(ctx)->template shaped<K, 2>({ 1, key_shape_.num_elements() });
        const auto key_buckets_tensor = key_buckets_.AccessTensor(ctx)->template matrix<K>();
    ...
    Status CheckKeyAndValueTensorsForImport(const Tensor &keys, const Tensor &values) override
    {
        ...
        if (values.shape() != expected_value_shape) {
            return errors::InvalidArgument("Expected shape ", expected_value_shape.DebugString(), " for value, got ",
    ...
    {
        tf_shared_lock l(mu_);
        return sizeof(MutableDenseHashTable) + key_buckets_.AllocatedBytes() + value_buckets_.AllocatedBytes() +
            empty_key_.AllocatedBytes();
    }

private:
    Status DoInsert(OpKernelContext *ctx, const Tensor &key, const Tensor &value, bool ignore_empty_and_deleted_key)
        EXCLUSIVE_LOCKS_REQUIRED(mu_)
    {
        ...
        auto key_buckets_matrix = key_buckets_.AccessTensor(ctx)->template matrix<K>();
        auto value_buckets_matrix = value_buckets_.AccessTensor(ctx)->template matrix<V>();
        const auto empty_key_tensor = empty_key_.AccessTensor(ctx)->template shaped<K, 2>({ 1, key_size });
        const auto deleted_key_tensor = deleted_key_.AccessTensor(ctx)->template shaped<K, 2>({ 1, key_size });
        const int64 bit_mask = num_buckets_ - 1;
        for (int64 i = 0; i < num_elements; ++i) {
            const uint64 key_hash = HashKey(key_matrix, i);
            if (empty_key_hash_ == key_hash && IsEqualKey(empty_key_tensor, 0, key_matrix, i)) {
                if (ignore_empty_and_deleted_key) {
                    continue;
                }
                return errors::InvalidArgument("Using the empty_key as a table key is not allowed");
            }
            if (deleted_key_hash_ == key_hash && IsEqualKey(deleted_key_tensor, 0, key_matrix, i)) {
                if (ignore_empty_and_deleted_key) {
                    continue;
                }
                return errors::InvalidArgument("Using the deleted_key as a table key is not allowed");
            ...
            while (true) {
                if (IsEqualKey(key_buckets_matrix, bucket_index, key_matrix, i)) {
                    for (int64 j = 0; j < value_size; ++j) {
                        value_buckets_matrix(bucket_index, j) = SubtleMustCopyIfIntegral(value_matrix(i, j));
                    }
                    break;
                }
                if (IsEqualKey(key_buckets_matrix, bucket_index, empty_key_tensor, 0) ||
                    IsEqualKey(key_buckets_matrix, bucket_index, deleted_key_tensor, 0)) {
                    ++num_entries_;
                    for (int64 j = 0; j < key_size; ++j) {
                        key_buckets_matrix(bucket_index, j) = SubtleMustCopyIfIntegral(key_matrix(i, j));
                    }
                    for (int64 j = 0; j < value_size; ++j) {
                        value_buckets_matrix(bucket_index, j) = SubtleMustCopyIfIntegral(value_matrix(i, j));
                    }
                ...
                bucket_index = (bucket_index + num_probes) & bit_mask; // quadratic probing
                if (num_probes >= num_buckets_) {
                    return errors::Internal("Internal error in MutableDenseHashTable insert");
                }
            }
        }
        return Status::OK();
    ...
    Status DoRemove(OpKernelContext *ctx, const Tensor &key) EXCLUSIVE_LOCKS_REQUIRED(mu_)
    {
        ...
        const auto empty_key_tensor = empty_key_.AccessTensor(ctx)->template shaped<K, 2>({ 1, key_size });
        const auto deleted_key_tensor = deleted_key_.AccessTensor(ctx)->template shaped<K, 2>({ 1, key_size });
        const auto deleted_key_flat = deleted_key_.AccessTensor(ctx)->template flat<K>();
        const int64 bit_mask = num_buckets_ - 1;
        for (int64 i = 0; i < num_elements; ++i) {
            const uint64 key_hash = HashKey(key_matrix, i);
            if (empty_key_hash_ == key_hash && IsEqualKey(empty_key_tensor, 0, key_matrix, i)) {
            ...
            while (true) {
                if (IsEqualKey(key_buckets_matrix, bucket_index, key_matrix, i)) {
                    --num_entries_;
                    for (int64 j = 0; j < key_size; ++j) {
                        key_buckets_matrix(bucket_index, j) = SubtleMustCopyIfIntegral(deleted_key_flat(j));
                    }
                    break;
                }
                if (IsEqualKey(key_buckets_matrix, bucket_index, empty_key_tensor, 0)) {
                ...
                bucket_index = (bucket_index + num_probes) & bit_mask; // quadratic probing
                if (num_probes >= num_buckets_) {
                    return errors::Internal("Internal error in MutableDenseHashTable remove");
                }
            }
        }
        return Status::OK();
    ...
    Status AllocateBuckets(OpKernelContext *ctx, int64 new_num_buckets) EXCLUSIVE_LOCKS_REQUIRED(mu_)
    {
        if (new_num_buckets < 4 || ((new_num_buckets & (new_num_buckets - 1)) != 0)) {
            return errors::InvalidArgument("Number of buckets must be at least 4 and a power of 2, got: ",
                new_num_buckets);
        }
        ...
        TF_RETURN_IF_ERROR(ctx->allocate_persistent(key_dtype(), TensorShape({ num_buckets_, key_size }), &key_buckets_,
            &key_buckets_tensor));
        auto key_buckets_matrix = key_buckets_tensor->matrix<K>();
        const auto empty_key_flat = empty_key_.AccessTensor(ctx)->template flat<K>();
        for (int64 i = 0; i < num_buckets_; ++i) {
            for (int64 j = 0; j < key_size; ++j) {
        ...
        TF_RETURN_IF_ERROR(ctx->allocate_persistent(value_dtype(), TensorShape({ num_buckets_, value_size }),
            &value_buckets_, &value_buckets_tensor));
    ...
    Status Rebucket(OpKernelContext *ctx, int64 num_new_buckets) EXCLUSIVE_LOCKS_REQUIRED(mu_)
    {
    ...
    bool IsEqualKey(typename TTypes<K>::Matrix tensor1, int64 index1, MT2 tensor2, int64 index2) const
    {
        for (int64 i = 0; i < key_shape_.num_elements(); ++i) {
...
template <>
Status MutableDenseHashTable<long long, float>::Find(OpKernelContext *ctx, const Tensor &key, Tensor *value,
    const Tensor &default_value) LOCKS_EXCLUDED(mu_)
{
    const int64 num_elements = (key.dims() == 0) ? 1 : key.dim_size(0);
    const int64 key_size = key_shape_.num_elements();
    const int64 value_size = value_shape_.num_elements();
    ...
    const auto key_matrix = key.shaped<long long, 2>({ num_elements, key_size });
    auto value_matrix = value->shaped<float, 2>({ num_elements, value_size });
    const auto default_flat = default_value.flat<float>();

    tf_shared_lock l(mu_);
    const auto key_buckets_matrix = key_buckets_.AccessTensor(ctx)->template matrix<long long>();
    const auto value_buckets_matrix = value_buckets_.AccessTensor(ctx)->template matrix<float>();
    const auto empty_key_matrix = empty_key_.AccessTensor(ctx)->template shaped<long long, 2>({ 1, key_size });
    const auto deleted_key_matrix = deleted_key_.AccessTensor(ctx)->template shaped<long long, 2>({ 1, key_size });
    if (key_size <= 1) {
        ktfop::TableInfo info{};
        info.numBuckets = num_buckets_;
        info.valueSize = value_size;
        info.emptyKey = empty_key_matrix(0);
        info.deletedKey = deleted_key_matrix(0);
        info.keyBucket = reinterpret_cast<int64_t *>(&key_buckets_matrix(0, 0));
        info.valueBucket = &value_buckets_matrix(0, 0);

        const auto *keys = reinterpret_cast<const int64_t *>(&key_matrix(0, 0));
        float *values = &value_matrix(0, 0);
        const float *default_values = &default_flat(0);

        if (ktfop::Find(info, keys, values, default_values, num_elements) != 0) {
            return errors::InvalidArgument("Find error");
        }
        return Status::OK();
    }
    const int64 bit_mask = num_buckets_ - 1;
...
public:
    ...
    void Compute(OpKernelContext *ctx) override
    {
        ...
        DataType expected_input_0 = (ctx->input_dtype(0) == DT_RESOURCE) ? DT_RESOURCE : DT_STRING_REF;
        DataTypeVector expected_inputs = { expected_input_0, table->key_dtype(), table->value_dtype() };
...
REGISTER_KERNEL_BUILDER(Name("LookupTableFind").Device(DEVICE_CPU), LookupTableFindOp);
REGISTER_KERNEL_BUILDER(Name("LookupTableFindV2").Device(DEVICE_CPU), LookupTableFindOp);
...
REGISTER_KERNEL_BUILDER(Name("LookupTableInsert").Device(DEVICE_CPU), LookupTableInsertOp);
REGISTER_KERNEL_BUILDER(Name("LookupTableInsertV2").Device(DEVICE_CPU), LookupTableInsertOp);
...
REGISTER_KERNEL_BUILDER(Name("LookupTableRemoveV2").Device(DEVICE_CPU), LookupTableRemoveOp);
...
REGISTER_KERNEL_BUILDER(Name("LookupTableSize").Device(DEVICE_CPU), LookupTableSizeOp);
REGISTER_KERNEL_BUILDER(Name("LookupTableSizeV2").Device(DEVICE_CPU), LookupTableSizeOp);
...
REGISTER_KERNEL_BUILDER(Name("LookupTableExport").Device(DEVICE_CPU), LookupTableExportOp);
REGISTER_KERNEL_BUILDER(Name("LookupTableExportV2").Device(DEVICE_CPU), LookupTableExportOp);
...
REGISTER_KERNEL_BUILDER(Name("LookupTableImport").Device(DEVICE_CPU), LookupTableImportOp);
REGISTER_KERNEL_BUILDER(Name("LookupTableImportV2").Device(DEVICE_CPU), LookupTableImportOp);

// Register the HashTable op with the currently supported key and value types.
#define REGISTER_KERNEL(key_dtype, value_dtype)                                           \
    REGISTER_KERNEL_BUILDER(Name("HashTable")                                             \
                                .Device(DEVICE_CPU)                                       \
                                .TypeConstraint<key_dtype>("key_dtype")                   \
                                .TypeConstraint<value_dtype>("value_dtype"),              \
        LookupTableOp<lookup::HashTable<key_dtype, value_dtype>, key_dtype, value_dtype>) \
...
#define REGISTER_KERNEL(key_dtype, value_dtype)                                                           \
    REGISTER_KERNEL_BUILDER(Name("MutableHashTable")                                                      \
                                .Device(DEVICE_CPU)                                                       \
                                .TypeConstraint<key_dtype>("key_dtype")                                   \
                                .TypeConstraint<value_dtype>("value_dtype"),                              \
        LookupTableOp<lookup::MutableHashTableOfScalars<key_dtype, value_dtype>, key_dtype, value_dtype>) \
...
#define REGISTER_KERNEL(key_dtype, value_dtype)                                                           \
    REGISTER_KERNEL_BUILDER(Name("MutableHashTableOfTensors")                                             \
                                ...
                                .TypeConstraint<key_dtype>("key_dtype")                                   \
                                .TypeConstraint<value_dtype>("value_dtype"),                              \
        LookupTableOp<lookup::MutableHashTableOfTensors<key_dtype, value_dtype>, key_dtype, value_dtype>) \
    REGISTER_KERNEL_BUILDER(Name("MutableHashTableOfTensorsV2")                                           \
                                .Device(DEVICE_CPU)                                                       \
                                .TypeConstraint<key_dtype>("key_dtype")                                   \
                                .TypeConstraint<value_dtype>("value_dtype"),                              \
        LookupTableOp<lookup::MutableHashTableOfTensors<key_dtype, value_dtype>, key_dtype, value_dtype>)
...
#define REGISTER_KERNEL(key_dtype, value_dtype)                                                       \
    REGISTER_KERNEL_BUILDER(Name("MutableDenseHashTable")                                             \
                                .Device(DEVICE_CPU)                                                   \
                                .TypeConstraint<key_dtype>("key_dtype")                               \
                                .TypeConstraint<value_dtype>("value_dtype"),                          \
        LookupTableOp<lookup::MutableDenseHashTable<key_dtype, value_dtype>, key_dtype, value_dtype>) \
    REGISTER_KERNEL_BUILDER(Name("MutableDenseHashTableV2")                                           \
                                .Device(DEVICE_CPU)                                                   \
                                .TypeConstraint<key_dtype>("key_dtype")                               \
                                .TypeConstraint<value_dtype>("value_dtype"),                          \
        LookupTableOp<lookup::MutableDenseHashTable<key_dtype, value_dtype>, key_dtype, value_dtype>)
